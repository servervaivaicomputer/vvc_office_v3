...
router.post('/admin/users', authenticate, requireAdmin, async function (req, res) {
  try {
    ...
    if (existing.error) {
      console.error('Check username error:', existing.error.message);
      return res.status(500).json({ error: 'Could not check username: ' + existing.error.message });
    }
    ...
    if (insertResult.error) {
      console.error('Create user database error:', insertResult.error.message);
      if (insertResult.error.code === '23505') return res.status(409).json({ error: 'Username exists.' });
      return res.status(500).json({ error: 'Create failed: ' + insertResult.error.message });
    }
    ...
    return res.status(201).json({
      success: true,
      user: { username, role, page_access: pageAccess, login_status: 'inactive' }
    });
  } catch (error) {
