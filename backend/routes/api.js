...
router.put('/admin/users/:id', authenticate, requireAdmin, async function(req, res) {
  try {
    ...
    if (req.body.pageAccess) updateData.page_access = req.body.pageAccess;
    if (req.body.password && req.body.password.length >= 8), pageAccess: result.data.page_access {
      updateData.password = await bcrypt.hash(req.body.password, 12);
    }
    var result = await supabase.from('users').update(updateData).eq('id', req.params.id).select('id, username, role, page_access, login_status').single();
    if (result.error    }
    await logActivity({ userId: req.user._id, username: req.user.username, action: 'access_granted', ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua, details: 'Updated ' + result.data.username });
...
router.post('/admin/users/:id/block', authenticate, requireAdmin, async function(req, res) {
  try {
    ...
    var userResult = await supabase.from('users').select('username'). });
  } catch (e) {
    res.status(500).json({ error: 'Server error' });
  }
});
...
eq('id', req.params.id).single();
    ...
    await logActivity({ userId: req.user._id, username: req.user.username, action: 'blocked', ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua, details: 'Blocked ' + name });
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: 'Server error' });
  }
...
router.post('/admin/users/:id/unblock', authenticate, requireAdmin, async function(req, res) {
  try {
    ...
    var name = userResult.data ? userResult.data.username : req.params.id;
    await logActivity({ userId: req.user._id, username: req.user.username, action: 'unblocked', ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua, details: 'Unblocked ' + name });
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: 'Server error' });
  }
...
router.put('/admin/users/:id/access', authenticate, requireAdmin, async function(req, res) {
  try {
    ...
    await logActivity({ userId: req.user._id, username: req.user.username, action: 'access_granted', ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua, details: details });
...
router.get('/admin/stats/pages', authenticate, requireAdmin, async function(_req, res) {
  try {
    ...
    for (var i = 0; i < pages.length; i++) {
      ...
      var mResult = await supabase.from('audit_logs').select('id', { count: 'exact', head: true }).eq('action', 'page_view').eq('page', p).gte('timestamp', d30);
...
router.get('/admin/logs', authenticate, requireAdmin, async function(req, res) {
  try {
    ...
    if (req.query.username) query = query.ilike('username', '%' + req.query.username + '%');
