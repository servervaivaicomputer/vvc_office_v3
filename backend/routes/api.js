...
router.post('/auth/login', loginLimiter, async (req, res) => {
  try {
    ...
    /* Blocked? */
    if (user.is_blocked) {
      await logActivity({ userId: user.id, username: user.username, action: 'login_failed', ip, device: dev.name, userAgent: dev.ua, status: 'blocked', details: 'Blocked account attempt' });
      return res.status(403).json({ error: 'Account blocked. Contact admin.' });
    }

    /* Device blocked? */
    const { data: devBlocked } = await supabase
      .from('devices')
      .select('id')
      .eq('user_id', user.id)
      .eq('ip', ip)
    ...
    return res.json({
      success: true,
      token,
      user: { username: user.username, role: user.role, pageAccess: user.page_access || [] }
    });
...
router.post('/auth/logout', authenticate, async (req, res) => {
  await logActivity({
    ...
    ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua, status: 'success'
  });
...
router.get('/auth/me', authenticate, (req, res) => {
  res.json({
    user: {
      ...
      pageAccess: req.user.pageAccess, lastLogin: req.user.lastLogin, loginStatus: req.user.loginStatus
    }
  });
});
...
router.get('/pages/:pageName', authenticate, async (req, res) => {
  try {
    ...
    await logActivity({
      userId: req.user._id, username: req.user.username, action: 'page_view', page,
      ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua, status: 'success'
    });
...
router.get('/admin/dashboard', authenticate, requireAdmin, async (_req, res) => {
  try {
    ...
    const [
      ...
      supabase.from('audit_logs').select('user_id', { count: 'exact' }).eq('action', 'login').gte('timestamp', today),
...
router.post('/admin/users/:id/block', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    await logActivity({
      ...
      ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua,
      details: `Blocked ${u?.username || req.params.id}`
    });
...
router.post('/admin/users/:id/unblock', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    await logActivity({
      ...
      ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua,
      details: `Unblocked ${u?.username || req.params.id}`
    });
...
router.put('/admin/users/:id/access', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    await logActivity({
      userId: req.user._id, username: req.user.username, action: 'access_granted',
      ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua,
      details: `Access for ${u.username}: ${JSON.stringify(u.page_access)}`
    });
...
router.get('/admin/stats/pages', authenticate, requireAdmin, async (_req, res) => {
  try {
    ...
    /* Daily aggregation — fetch raw and group in JS */
