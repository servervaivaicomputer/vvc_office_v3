  ...
  try {
    ...
    if (user.isBlocked) {
      await logActivity({
        ...
        ip, device: dev.name, userAgent: dev.ua, status: 'blocked',
        details: 'Attempt on blocked account'
      });
    ...
    if (!ok) {
      ...
      if (user.failedLoginAttempts >= max) {
        ...
        user.blockedReason   = `Auto-blocked after ${max} failed attempts`;
        user.loginStatus     = 'blocked';
        await logActivity({
          userId: user._id, username: user.username, action: 'blocked',
          ip, device: dev.name, userAgent: dev.ua, status: 'blocked',
      ...
      await logActivity({
        userId: user._id, username: user.username, action: 'login_failed',
        ip, device: dev.name, userAgent: dev.ua, status: 'failure',
        details: `Attempt ${user.failedLoginAttempts}/${max}`
      });

      return res.status(401).json({
        error: 'Invalid credentials.',
        attemptsRemaining: Math.max(0, max - user.failedLoginAttempts)
      });
    }
    ...
    user.lastLoginIP     = ip;
    user.lastLoginDevice = dev.name;
    user.loginStatus     = 'active';
    await recordDevice(user, ip, dev);

    const token = generateToken(user);
    await logActivity({
      userId: user._id, username: user.username, action: 'login',
      ip, device: dev.name, userAgent: dev.ua, status: 'success',
      details: 'Login OK'
    });
    ...
    return res.json({
      success: true,
      token,
      user: { username: user.username, role: user.role, pageAccess: user.pageAccess }
    });
  ...
  try {
    ...
    if (!ok) {
      ...
      if (user.failedLoginAttempts >= max) {
        ...
        user.blockedBy     = 'auto';
        user.blockedAt     = new Date();
        user.blockedReason = 'Auto-blocked';
        user.loginStatus   = 'blocked';
      }
...
/* ================================================================
   PAGE CONTENT API  ★ NEW ★
   ================================================================ */

/* GET /api/pages/:pageName — protected page HTML return করে */
router.get('/pages/:pageName', authenticate, async (req, res) => {
  try {
    ...
    console.error('Page serve error:', e);
    res.status(500).json({ error: 'Server error' });
  }
});

/* GET /api/pages/admin/panel — admin panel HTML */
...
router.get('/admin/dashboard', authenticate, requireAdmin, async (_req, res) => {
  try {
    ...
    res.json({
      stats: { totalUsers, blockedUsers, activeToday, viewsToday, views7, views30, loginsToday, failsToday },
      recent
    });
  } catch (e) {
    console.error(e);
...
router.get('/admin/users', authenticate, requireAdmin, async (_req, res) => {
  try {
    res.json({ users: await User.find().select('-password').sort({ createdAt: -1 }).lean() });
  } catch (e) {
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.post('/admin/users', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    await logActivity({
      userId: req.user._id, username: req.user.username, action: 'user_created',
      ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua,
      details: `Created ${u.username}`
    });
...
router.put('/admin/users/:id', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    await user.save();
    await logActivity({
      userId: req.user._id, username: req.user.username, action: 'access_granted',
      ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua,
...
router.delete('/admin/users/:id', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    await logActivity({
      userId: req.user._id, username: req.user.username, action: 'user_deleted',
      ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua,
      details: `Deleted ${u.username}`
    });
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: 'Server error' });
...
router.post('/admin/users/:id/block', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    u.blockedReason  = req.body.reason || 'Blocked by admin';
    u.loginStatus    = 'blocked';
    await u.save();
    await logActivity({
      userId: req.user._id, username: req.user.username, action: 'blocked',
      ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua,
      details: `Blocked ${u.username}`
    });
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: 'Server error' });
...
router.post('/admin/users/:id/unblock', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    u.loginStatus         = 'inactive';
    await u.save();
    await logActivity({
      userId: req.user._id, username: req.user.username, action: 'unblocked',
      ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua,
      details: `Unblocked ${u.username}`
    });
    res.json({ success: true });
  } catch (e) {
    res.status(500).json({ error: 'Server error' });
...
router.put('/admin/users/:id/access', authenticate, requireAdmin, async (req, res) => {
  try {
    const u = await User.findById(req.params.id);
    if (!u) return res.status(404).json({ error: 'Not found' });
    u.pageAccess = req.body.pageAccess || [];
    await u.save();
    await logActivity({
      userId: req.user._id, username: req.user.username, action: 'access_granted',
      ip: req.clientIP, device: req.deviceInfo.name, userAgent: req.deviceInfo.ua,
      details: `Access for ${u.username}: [${u.pageAccess}]`
    });
...
router.post('/admin/devices/:uid/:did/block', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    if (!u) return res.status(404).json({ error: 'Not found' });
    const d = u.devices.id(req.params.did);
